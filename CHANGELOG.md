...
## 第13弾（チュートリアルの2周目：実際に通して直した）
`TUTORIAL.md` を「初めて読む人」と同じ手順で頭から実行し、**動かなかった箇所を6件修正**した。
- **ステップ6のクローン手順が失敗する**：既定ブランチ（`main`）は別プロジェクトで `tools/` も `TUTORIAL.md` も存在しない。`-b claude/fable-ui-operations-guide-8ehgke` の指定を追加し、マージ後は不要になる旨を注記
- **ステップ8のフォルダ選択の対象が違う**：`docs/ui-guide` ではなく**リポジトリのルート**（ボタン文言も実物の「📁 フォルダを選択（リポジトリのルート）」に合わせた）。`file://` で「🌐 このページから読み込む」を押したときの挙動も明記
- **ステップ3の場所の説明が違う**：`const TOTAL = 100000` は「1行目」ではなく `(() => {` の次の行の複数宣言の中にある
- **ステップ2に決定的な確認を追加**：折りたたみの表示コードと、実行された `<script>` の中身が同一の文字列であることを1行で確かめられるようにした（表示時にインデントを整えるため、空白を潰して比較する）
- **依存の要否を明記**：`build:docs` と `check:ime` は `npm install` 不要で動く。`npm test` / `test:docs` だけ Playwright と Chromium が要る（無いと `Cannot find module 'playwright'` で止まる）
- **ステップ4に手順を追加**：まず手元の PC で一周してから現場端末で比べる。現場端末には単一ファイル版1枚を渡せばよい
- 検証：チュートリアルの手順が実物と合っているかを固定する回帰テストを追加（表示コード＝実行コードの確認、TOTAL の行位置、バンドラのボタン文言）。**130件**
