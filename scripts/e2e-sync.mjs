...
try {
  ...
  // 一覧: キーボード移動・一括変更・納期超過の強調・集計
  {
    ...
    // 範囲選択 → コピー(タブ区切り)→ 貼り付け(複数行 / 範囲埋め / 読み取り専用列は飛ばす / 日付の正規化)
    const clip = (page, type, text) => page.evaluate(([type, text]) => { const dt = new DataTransfer(); if (text) dt.setData('text/plain', text); const ev = new ClipboardEvent(type, { clipboardData: dt, bubbles: true, cancelable: true }); document.activeElement.dispatchEvent(ev); return { text: dt.getData('text/plain'), prevented: ev.defaultPrevented } }, [type, text])
    await a.locator('[data-testid="sheet-row"] input[data-col="partNo"]').first().focus()
    await a.keyboard.press('Shift+ArrowDown')
    await a.keyboard.press('Shift+ArrowRight')
    ok('Shift+矢印で 2 行 × 2 列の範囲', (await a.locator('td[data-range="true"]').count()) === 4)
    const copied = await clip(a, 'copy')
    ok(`Ctrl+C で範囲がタブ区切りになる (${copied.text.replace(/\t/g, '⇥').replace(/\n/g, '⏎')})`, copied.prevented && copied.text.split('\n').length === 2 && copied.text.split('\n').every((l) => l.split('\t').length === 2) && copied.text.includes('SHEET-EDIT'))
    await a.locator('[data-testid="sheet-row"] input[data-col="assignee"]').first().focus()
    const pasted = await clip(a, 'paste', '田中\n高橋')
    ok('複数行の貼り付けが下の行へ順に入る', pasted.prevented && (await waitFor(() => a.evaluate(() => { const cs = window.__qcEditor.getShapes().filter((s) => s.type === 'request-card').map((c) => c.assignee).sort(); return cs.join(',') === '田中,高橋' }))))
    ok('貼り付けは相手にも届く', await waitFor(() => b.evaluate(() => window.__qcEditor.getShapes().filter((s) => s.type === 'request-card').every((c) => c.assignee === '田中' || c.assignee === '高橋'))))
    await a.locator('[data-testid="sheet-row"] input[data-col="note"]').first().focus()
    await a.keyboard.press('Shift+ArrowDown')
    await clip(a, 'paste', '一括メモ')
    ok('1 値を範囲に貼ると全セルが埋まる', await waitFor(() => a.evaluate(() => window.__qcEditor.getShapes().filter((s) => s.type === 'request-card').every((c) => c.note === '一括メモ'))))
    await a.locator('[data-testid="sheet-row"] input[data-col="resultNote"]').first().focus()
    await clip(a, 'paste', '所見A\t判定者は書けない\n所見B\t同上')
    ok('読み取り専用の列(判定者)は飛ばされ、所見だけ入る', await waitFor(() => a.evaluate(() => { const cs = window.__qcEditor.getShapes().filter((s) => s.type === 'request-card'); return cs.every((c) => c.judgedBy === '') && cs.map((c) => c.resultNote).sort().join(',') === '所見A,所見B' })))
    await a.locator('[data-testid="sheet-row"] input[data-col="dueDate"]').first().focus()
    await clip(a, 'paste', '2026/12/1\n12/25/2026')
    ok('日付は貼り付け時に YYYY-MM-DD へ正規化', await waitFor(() => a.evaluate(() => window.__qcEditor.getShapes().filter((s) => s.type === 'request-card').map((c) => c.dueDate).sort().join(',') === '2026-12-01,2026-12-25')))
    const singleOnly = await clip(a, 'paste', 'そのまま')
    ok('1 セルへの通常の貼り付けはブラウザに任せる', singleOnly.prevented === false)
    await a.keyboard.press('Escape')
