...
| 05 | `05-advanced.html` | D層（第3弾）業務アプリの設計パターン 8項目：大規模グリッド複合(71)、検索の型(74)、フォームの型(75)、バッチ操作(76)、差分表示(77)、ドッキング完成形(80)、ワークスペース永続化(81)、マルチウィンドウ同期(85) | 完成 |
...
- D層の番号（71〜85）は第3弾提案時のリスト番号をそのまま使っており、飛び番がある（63〜70 の演出系、72・73・78・79・82〜84・86 は未作成）
- 75・80・81 は `localStorage` に下書き・レイアウト・作業状態を保存する（キーは `ui-guide.*`）。85 は `BroadcastChannel` と `storage` イベントの2経路で同期する
