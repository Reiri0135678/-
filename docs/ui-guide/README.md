...
| - | `../ui-kit/` | デモから切り出した再利用部品（15モジュール、テスト26件） | 完成 |
...
- 08 基礎知識に F9（レイアウトスラッシング）・F10（計測の道具）を追加。F4 は仕様上、passive リスナー内の preventDefault でコンソールに警告が出る（意図した実演）
- 01 カタログは印刷ボタンでフィルタ結果を早見表として印刷できる
