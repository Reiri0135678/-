...
- server は **tsx で TypeScript を直接実行**する（`npm start`）。`shared` を web と server がソースのまま共有するため、tsc の出力を Node で動かすとモジュール解決が壊れる。ビルド工程を減らす方を優先した。負荷が問題になったら `shared` を別途ビルドして切り替える。
